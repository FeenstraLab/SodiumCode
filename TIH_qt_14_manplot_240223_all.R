...
setwd("TIH_quantitative_GWAS202308")
...
df <-  fread2(opt$gwas) %>% filter(TEST == "ADD-INT_SNPxdrug=THI",
                                   INFO > 0.4)
...
gwas_padj$TEST <-"ADD-INT_SNPxdrug=THI"

gwas_padj_INT_1 <- gwas_padj
...
df <-  fread2(opt$gwas) %>% filter(TEST == "ADD-INT_SNP",
                                   INFO > 0.4)
...
gwas_padj$TEST <-"ADD-INT_SNP"

gwas_padj_INT_2 <- gwas_padj

## 3 ----
cat(paste0("[", format(Sys.time(), "%X"), "] ",
           "Reading input file ", opt$gwas," ..."))

df <-  fread2(opt$gwas) %>% filter(TEST == "ADD-INT_drug=THI",
                                   INFO > 0.4)
...
gwas_padj$TEST <-"ADD-INT_drug=THI"

gwas_padj_INT_3 <- gwas_padj

## 4 ----
cat(paste0("[", format(Sys.time(), "%X"), "] ",
           "Reading input file ", opt$gwas," ..."))

df <-  fread2(opt$gwas) %>% filter(TEST == "ADD-INT_2DF",
                                   INFO > 0.4)
...
gwas_padj$TEST <-"ADD-INT_2DF"

gwas_padj_INT_4 <- gwas_padj

## 5 ----
cat(paste0("[", format(Sys.time(), "%X"), "] ",
           "Reading input file ", opt$gwas," ..."))

df <-  fread2(opt$gwas) %>% filter(TEST == "ADD",
                                   INFO > 0.4)
...
gwas_padj_INT_5 <- gwas_padj

## combining -------
gwas_padj <- gwas_padj_INT_1 %>% 
  rbind(gwas_padj_INT_2,
        gwas_padj_INT_3,
        gwas_padj_INT_4,
        gwas_padj_INT_5)
